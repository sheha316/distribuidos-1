...
func (s *Server) CrearOrdenPyme(ctx context.Context, request *Request_CrearOrdenPyme) (*Response_CrearOrden, error) {
  ...
  var guardar = [][]string{
    {request.Id,request.Producto,strconv.Itoa(int(request.Valor)),request.Tienda,request.Destino,strconv.Itoa(int(request.Prioritario)),"En bodega"},
  }
...
func (s *Server) CrearOrdenRetail(ctx context.Context, request *Request_CrearOrdenRetail) (*Response_CrearOrden, error) {
  ...
  writer:=csv.NewWriter(file)
  var guardar = [][]string{
    {request.Id,request.Producto,strconv.Itoa(int(request.Valor)),request.Tienda,request.Destino,"En bodega"},
  }
  erros=writer.WriteAll(guardar)
